...
export default function DetailProduct({ navigation, route }) {
  ...
  const handleKeranjang = useCallback(() => {
    ...
    if (cekUser.length == 1) {
      let setupData;
      let dataKeranjang = keranjangGlobalState;
      let idPerProduct = `${data.id}-${coffeeSize[activeIndexSize]}-${coffeeTemperature[activeIndexTemperature]}`;

      const foundProductIndex = dataKeranjang.findIndex(
        (product) => product.idPerProduct == idPerProduct
      );

      if (foundProductIndex !== -1) {
        dataKeranjang[foundProductIndex].quantity =
          dataKeranjang[foundProductIndex].quantity + quantity;
        dataKeranjang[foundProductIndex].totalPriceProduct =
          dataKeranjang[foundProductIndex].totalPriceProduct + price;
      } else {
        setupData = {
          idPerProduct,
          ...data,
        ...
        dataKeranjang.push(setupData);
      }

      dispatch({ type: "SET_KERANJANG", payload: dataKeranjang });
